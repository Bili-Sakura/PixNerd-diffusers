...
class PixNerdTransformer2DModel(ModelMixin, ConfigMixin):
    """
    Diffusers wrapper around the PixNerd class-conditional DiT denoiser.
    """
    ...
    def patch_size(self) -> int:
        return int(self.denoiser.patch_size)

    @property
    def in_channels(self) -> int:
        return int(self.denoiser.in_channels)
    ...
    def get_inference_denoiser(self, use_ema: bool = True) -> nn.Module:
    ...
    ) -> Union[PixNerdTransformer2DModelOutput, Tuple[torch.Tensor]]:
        batch_size = sample.shape[0]
        t = self._as_timestep_tensor(timestep, batch_size, sample.device)
        out = self.get_inference_denoiser(use_ema=True)(sample, t, encoder_hidden_states)
        if not return_dict:
            return (out,)
